"""Pull 7 million Gaia DR3 stars — the same quality cut as the shipped 500k, 14x more of them.
...
The cut is unchanged from prepare_universe_data.load_stars_gaia:
    ...
    phot_g_mean_mag < 18        detected well, not scraped from the noise floor

74,373,115 stars in DR3 satisfy it. The shipped layer uses 500,000 of them — 0.7%.
So this is not a relaxation to inflate a number; it is more of exactly the same thing.
...
CUT = ("parallax>0.5 AND parallax_over_error>10 AND phot_g_mean_mag<18 "
       "AND l IS NOT NULL AND b IS NOT NULL")
TARGET = 7_000_000
BANDS = 20
RI_MAX = 1_811_709_770          # MAX(random_index) over gaiadr3.gaia_source
POP = 74_373_115                # rows satisfying CUT — measured, not assumed
OUT = "gaia_points.csv"
...
def main():
    ...
    span = int(RI_MAX * (TARGET / POP) * 1.06)
    width = span // BANDS
